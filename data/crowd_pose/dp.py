...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, config, ds, index):
        ...
        self.separateGenerateHeatmap = GenerateSeparateHeatmap(config['train']['output_res'], config['inference']['num_parts'])
    ...
    def loadImage(self, idx):
        ...
        if False and np.random.randint(2) == 0:
            inp = inp[:, ::-1]
            mask = mask[:, ::-1]
        ...
        return self.preprocess(inp).astype(np.float32), mask.astype(np.float32), keypoints.astype(
            np.int32), heatmaps, separate_heatmaps.astype(np.float32), im_name
...
def init(config):
    ...
    del (dataset['valid'])
    for key in dataset:
        loaders[key] = torch.utils.data.DataLoader(dataset[key], batch_size=batchsize, shuffle=True, num_workers=config['train']['num_workers'], pin_memory=False, worker_init_fn=11)
    ...
    def gen(phase):
        ...
        for i in range(batchnum):
            ...
            imgs, masks, keypoints, heatmaps, separate_heatmaps, im_name = next(loader)
            yield {
                'imgs': imgs,
                'masks': masks,
                'heatmaps': heatmaps,
