...
def singleTagLoss(pred_tag, keypoints):
    ...
    if len(tags) == 0:
        return make_input(torch.full([], 0).float()), make_input(torch.full([], 0).float())
    ...
    diff = A - B
    # diff = torch.pow(diff, 2).sum(dim=2, keepdim=True)[:, :, 0]
...
class AEloss(nn.Module):
    def forward(self, tags, keypoints):
        push, pull = tagLoss(tags, keypoints)

        return torch.stack([torch.unsqueeze(push, 1), torch.unsqueeze(pull, 1)], dim=1)
