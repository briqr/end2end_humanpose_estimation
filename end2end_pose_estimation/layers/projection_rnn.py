...
class Projection(nn.Module):
    ...
    def __init__(self):
        ...
        self.softmax_op = nn.LogSoftmax(dim=-1)
        self.alpha = 4
    ...
    def multiplyByPeaked(self, x):
        #print('***x.shape)
        batch_size = x.shape[0] # this is the number of persons, not the batch size
   	#nu_joints = 17
        #nu_persons = x.shape[1] // nu_joints
        dim1 = x.shape[2]
        dim2 = x.shape[3]
        #x = x.reshape(batch_size, 17, dim1, dim2)
        alpha = self.alpha
        total_result = torch.tensor(x)
        for i in range(x.shape[0]):
            V_im = alpha * x[i]
            V_im = V_im.reshape(V_im.shape[0], dim1 * dim2)
            V_im = self.softmax_op(V_im) #*V_im
            V_im = V_im.reshape(V_im.shape[0], dim1, dim2)
            #print(V_im)
            #heatmap_size = V_im.shape[1] * V_im.shape[2]
            total_result[i] =  V_im
